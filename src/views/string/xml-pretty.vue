  ...
  <div class="container">
    ...
    <el-form ref="inputForm" :label-position="labelPosition" label-width="80px" :model="formLabelAlign">
      <el-form-item label="xml">
        <el-input v-model="input" type="textarea" :rows="4" placeholder="请输入需要美化的XML字符串" />
      </el-form-item>
      <el-form-item>
    ...
    <el-form ref="outputForm" :label-position="labelPosition" label-width="80px" :model="formLabelAlign">
      <el-form-item label="pretty xml">
        <el-input id="output" v-model="output" disabled type="textarea" :rows="4" placeholder="经过美化的XML字符串" />
      </el-form-item>
      <el-form-item>
...
export default {
  ...
  methods: {
    onSubmit() {
      const url = `${this.$main}/string/xml/pretty?input=${encodeURIComponent(this.input)}`
      fetch(url)
